...
export default function AboutHero() {
  return (
    <div className="bg-[#F6F6F0] md:h-[761px] h-[500px] hav flex justify-between items-center text-black px-3 overflow-hidden">
      <motion.div
        className="px-2 md:px-10 mt-50 md:mt-40 w-full flex z-10 flex-col gap-3.5"
        initial="initial"
        whileInView="animate"
        viewport={{ once: true }}
        variants={textAnimation}
      >
        <motion.p variants={textAnimation} className=" hidden md:flex">
          ABOUT US
        </motion.p>
        <motion.h3
          className="text-3xl md:text-[64px] font-bold lg:w-[977px] md:leading-20"
          variants={textAnimation}
        >
        ...
        <motion.div variants={imageAnimation} className="mt-[-100px]">
          <Image src={simp} alt="founder" className="w-[230px] object-cover" />
        </motion.div>
      </motion.div>
      <motion.div
      ...
      >
        <Image src={founder} alt="founder" className="w-[521px] object-cover" />
      </motion.div>
    </div>
  );
}
